...
- The `-c` option of `gcc/g++` is used specifically to transform the source files into object files without linking them. It doesn't matter which extension the source file has, that is, `-c` accepts both C (`.c`) or Assembly (`.s` or `.asm`) code.
